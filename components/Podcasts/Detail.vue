<template>
  <v-container>
    <v-row>
      <v-col cols="12">
        <v-card
          color="#952175"
          theme="dark"
          v-if="url !== '' && url !== undefined"
        >
          <div class="d-flex flex-no-wrap justify-space-between">
            <div>
              <v-card-title class="text-h5"> Podcast </v-card-title>

              <v-card-subtitle>{{ narrative }} : {{ url }} </v-card-subtitle>

              <v-card-actions>
                <audio controls>
                  <source
                    :src="`/resources/${narrative}/${url}`"
                    type="audio/wav"
                  />
            ...
            <v-avatar class="ma-3" size="125" rounded="0">
              <v-img :src="`/icons/${format}.png`"></v-img>
            </v-avatar>
          </div>
          <v-card-actions>
            <v-btn @click="toReturn">
              Regresar a listado de podcasts de {{ narrative }}
            </v-btn>
          </v-card-actions>
        </v-card>
      </v-col>
    ...
    <!--v-row>
            <v-col cols="12">
                <v-card class="mx-auto" color="#590040" variant="outlined">
                    <v-card-text class="text-h5">
                        <v-icon color="#C168A8" class="me-2">mdi-information</v-icon>
                        About this podcast
